...
abstract class Builder{
	...
	protected function getReadonly() {
		if ($this->readonly === NULL) {
			if (!empty(self::$_logs[$this->database->database()]) && ($useTables = $this->getUseTables())) {
				foreach (self::$_logs[$this->database->database()] as $table => $time) {
	...
	protected function setReadonly($ttl = 2) {
		foreach ($this->getUseTables() as $table) {
			self::$_logs[$this->database->database()][$table] = time() + ($ttl < 2 ? 2 : $ttl);
		}
	}
