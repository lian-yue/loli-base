...
class SQLBuilder extends Builder{
	...
	private function _query(array $querys, $having = NULL, array &$useTables, array &$useColumns, $logical = '') {
		...
		foreach ($querys as $column => &$query) {
			...
			switch ($compare) {
				case 'IN':
					// IN 运算符
					$value = array_map([$this->database, 'value'], !$function && isset($columnsType[$value->column]) ? array_map(self::$_typeFunctions[$columnsType[$value->column]], (array) $query->value) : (array) $query->value);
					// 空 的返回 1 = 2
					if (!$value) {
						$commands = ['1 = 2'];
						break 2;
				...
				default:
					...
					$arrays[] = ['compare' => $compare, 'value' => $this->database->value(!$function && isset($columnsType[$query->column]) ? call_user_func(self::$_typeFunctions[$columnsType[$query->column]], $query->value) : $query->value)];
			}
	...
	public function create() {
		if (empty($this->_data['create'])) {
			$table = $this->database->key($this->_table(), true);
	...
	public function insert() {
		if (empty($this->_data['insert'])) {
			...
			foreach ($this->documents as $document) {
				...
				foreach ($document as $name => $value) {
					...
					if (!$value instanceof Param) {
						$insert[$name] = $this->database->value(isset($columnsType[$name]) ? call_user_func(self::$_typeFunctions[$columnsType[$name]], $value) : $value);
						continue;
					}
					...
					if ($value->value instanceof Cursor) {
						...
						$insert[$name] = $this->database->value(isset($columnsType[$name]) ? call_user_func(self::$_typeFunctions[$columnsType[$name]], $value->value) : $value->value);
					}
			...
			$this->_data['insert'] = strtr($command, [':ignore' => $this->_ignore(), ':table' => $table, ':column' => $column, ':document' => $document]);
	...
	public function update() {
		if (empty($this->_data['update'])) {
			...
			foreach($this->documents[0] as $name => $value) {
				...
				if (!$value instanceof Param) {
					$document[$name] = $this->database->value(isset($columnsType[$name]) ? call_user_func(self::$_typeFunctions[$columnsType[$name]], $value) : $value);
					continue;
				}
				...
				if ($value->value instanceof Cursor) {
					...
					$data = $this->database->value(in_array($assignment, ['', '='], true) && isset($columnsType[$value->name]) ? call_user_func(self::$_typeFunctions[$columnsType[$value->name]], $value->value) : $value->value);
				}
			...
			$this->_data['update'] = strtr($command, [':ignore' => $this->_ignore(), ':form' => $this->_from('UPDATE'), ':document' => $document, ':where' => $this->_where(), ':order' => $this->_order(), ':offset' => $this->_offset(), ':limit' => $this->_limit()]);
	...
	public function select() {
		...
		if ($this->cache[0] && $this->_isCache && !$this->_lock() && (($this->_data['select'] = Cache::group('database.' . $this->database->database())->get($cacheKey = json_encode(['cache' => $this->cache, 'class' => $this->class] + $this->_data['selectReplaces']))) !== false && ($this->cache[1] < 1 || $this->cache[0] === -1 || (Cache::group('database.' . $this->database->database())->ttl($cacheKey) > $this->cache[1] || !Cache::group('database.' . $this->database->database())->add(true, 'TTL' . $cacheKey, $this->cache[1] + 1))))) {
			...
			// 读取数据
			$this->_data['select'] = $this->database->command($this->_data['selectCommand'], $this->getReadonly(), $this->class);

			// 写入缓存
			$this->cache[0] && (count($this->_data['select']) || $this->cache[2]) && !$this->_lock() && Cache::group('database.' . $this->database->database())->set($this->_data['select'], json_encode(['cache' => $this->cache, 'class' => $this->class] + $this->_data['selectReplaces']), $this->cache[0]);
	...
	public function count() {
		...
		if ($this->cache[0] && $this->_isCache && (($this->_data['count'] = Cache::group('database.' . $this->database->database())->get(json_encode(['cache' => $this->cache, 'class' => $this->class] + $this->_data['countReplaces']))) !== false && ($this->cache[1] < 1 || $this->cache[0] === -1 || (Cache::group('database.' . $this->database->database())->ttl($cacheKey) > $this->cache[1] || (Cache::group('database.' . $this->database->database())->ttl($cacheKey) > $this->cache[1] || !Cache::group('database.' . $this->database->database())->add(true, 'TTL' . $cacheKey,  $this->cache[1] + 1)))))) {
			// 用缓存的
		} else {
			// 需要读取数据的
			// rows 的
	...
	public function delete() {
		$command = strtr('DELETE :ignore :form :where :order :offset :limit;', [':ignore' => $this->_ignore(), ':form' => $this->_from('DELETE'), ':where' => $this->_where(), ':order' => $this->_order(), ':offset' => $this->_offset(), ':limit' => $this->_limit()]);
	...
	public function deleteCacheSelect($refresh = NULL) {
		$this->cache[0] && Cache::group('database.' . $this->database->database())->delete(json_encode(['cache' => $this->cache, 'class' => $this->class, ':field' => $this->_fields(), ':form' => $this->_from('SELECT'), ':where' => $this->_where(), ':group' => $this->_group('SELECT'), ':having' => $this->_having(), ':order' => $this->_order(), ':offset' => $this->_offset(), ':limit' => $this->_limit(), ':union' => $this->_union(), ':lock' => $this->_lock()]), $refresh === NULL ? $this->cache[1] : $refresh);
	...
	public function deleteCacheCount($refresh = NULL) {
		$this->cache[0] && Cache::group('database.' . $this->database->database())->delete(json_encode(['cache' => $this->cache, 'class' => $this->class, ':group' => $this->_group('COUNT'), ':form' => $this->_from('SELECT'), ':where' => $this->_where(), ':having' => $this->_having(), ':union' => $this->_union(), ':lock' => $this->_lock()]), $refresh === NULL ? $this->cache[1] : $refresh);
